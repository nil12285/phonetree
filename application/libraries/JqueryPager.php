...
namespace OneOneEight\AdminBundle\Library; 
...
class Jquerypager {
    ...
    {        
        ...
        $this->tableStart = "<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" id=\"".$this->idPager."\" class=\"table dataTable table-striped table-bordered $highLightClass\">\n";
        $this->tableEnd = "</table>";
    }
	...
	function addButton($type,$text="",$link="",$target='blank',$extra=array()){
		...
		$class="";
		switch ($type){
			case 'add':$class='icon-plus';break;
			case 'add_user':$class='icon-user';break;
			case 'save':$class='ui-icon-disk';break;            
            case 'trash':$class='icon-trash';break;
            case 'refresh':$class='icon-refresh';break;
            case 'mail':$class='icon-envelope';break;            
		}
        $uniqId = uniqid();
        if(!empty($type)){
            ...
            if($target == "overlay"){
                $this->olyFlag = true;
                $this->buttons .= "<a href='Javascript: void(0);' onClick=\"Javascript: openDialog('".$uniqId."')\" url=\"".$link."\" id=\"".$uniqId."\" title=\"".$text."\" class=\"\">";
            }
            elseif($target == "confirm"){
                $this->cnfFlag = true;
                $this->buttons .= "<a href='Javascript: void(0);' onClick=\"Javascript: openConfirm('".$uniqId."')\" url=\"".$link."\" id=\"".$uniqId."\" title=\"".$text."\" class=\"\">";
            }
            elseif($target == "popup"){                
                $this->buttons .= "<a href='Javascript: void(0);' onClick=\"Javascript: window.open('$link','$text','width=".$extra['width'].",height=".$extra['height'].",scrollbars=1,toolbar=0');\" id=\"".$uniqId."\" title=\"".$text."\" class=\"\">";
            }            
            else{
                $this->buttons .= "<a href='".$link."' target=\"_".$target."\" id=\"".$uniqId."\" title=\"".$text."\" class=\"\">";
            }
                        
            $this->buttons .= "<span class=\"".$class."\"></span>".$text;
            $this->buttons .= "</a>";
            $this->buttons .= "</li>";
        }        
	}
        ...
        foreach($data as $key=>$aRow)
    	{
            ...
            if(!empty($this->tbLink)){                        
                foreach($this->tbLink as $val2){
                  ...
                  array_push($aData,$this->_genrateLink($link,$aRow,$val2));
        		}
            }
    ...
    private function _getLinkHtml($linkData,$url,$data=array())
    {
        ...
        if(strtolower($linkData['text']) == "edit" || strtolower($linkData['text']) == "manage"){
            $linkHtml .= "<span class=\"icon-edit\"></span>".$linkData['text'];
        }
        elseif(strtolower($linkData['text']) == "delete" || strtolower($linkData['text']) == "remove"){
            $linkHtml .= "<span class=\"icon-trash\"></span>".$linkData['text'];
        }
        elseif(strtolower($linkData['text']) == "create" || strtolower($linkData['text']) == "add"){
            $linkHtml .= "<span class=\"icon-plus\"></span>".$linkData['text'];
        }
        elseif(strtolower($linkData['text']) == "download"){
            $linkHtml .= "<span class=\"icon-download\"></span>".$linkData['text'];
        }
        elseif(strtolower($linkData['text']) == "select"){
            $linkHtml .= "<span class=\"icon-ok\"></span>".$linkData['text'];
        }
        elseif(strtolower($linkData['text']) == "view"){
            $linkHtml .= "<span class=\"icon-eye-open\"></span>".$linkData['text'];
        }
        else{
